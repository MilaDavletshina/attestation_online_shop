...
class Contact(models.Model):
    ...
    street = models.CharField(
        max_length=100, verbose_name="Улица", help_text="Укажите название улицы"
    )
    house_number = models.CharField(
        max_length=100, verbose_name="Дом", help_text="Укажите номер дома"
    )
...
class Product(models.Model):
    ...
    model = models.CharField(
        max_length=100, verbose_name="Модель", help_text="Укажите модель"
    )
...
class NetworkLink(models.Model):
    ...
    name = models.IntegerField(
        choices=ORGANIZATION_TYPES, verbose_name="Тип организации"
    )
    ...
    products = models.ManyToManyField(
        Product, verbose_name="Продукт", help_text="Укажите продукт"
    )
    ...
    )  # Используем 'self' в ForeignKey для создания иерархической структуры
